...
public class welcome_controller implements Initializable {
    ...
    private String id = null, port = null, name = null, Seat = null, Total_Seat = null, check_excursion_ID, Text_Recommended_Place1 = null, Text_Recommended_Place2 = null;
...
//        home_search_result.getChildren().setAll(pane1);
        ...
        try {
            Connection myConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/group-8", "root", "");
            Statement statement = myConn.createStatement();
            ResultSet rs = statement.executeQuery("SELECT * FROM `excursions` WHERE `Name`= '" + auto_search.getText() + "'");
            while (rs.next()) {
                id = rs.getString("ID");
        ...
        if (welcomeController.isLoggedIn == false) {
            ...
            try {
                ...
                if (id.equals(check_excursion_ID)) {
                    ...
                    content.setBody(new Text("Sorry you have already booked this Excursion!"));
    ...
    public void initialize(URL location, ResourceBundle resources) {
        //Populating the random excursion in Recommended place
        try {
            Connection myConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/group-8", "root", "");
        ...
        //Ripple Effect for Recommended Places Pane
        ...
        //getting the array result SearchResult_AutoComplete function and binding it with the autocomplete textfield
        TextFields.bindAutoCompletion(auto_search, SearchResult_AutoComplete());
        if (isLoggedIn) {
            sample.Controller.Login_Controller login_controller;
            login_controller = new sample.Controller.Login_Controller();
